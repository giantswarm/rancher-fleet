...
cp "${VENDIR_SYNC_DIR}"/fleet-crd/templates/* "${CRD_CHART_DIR}"/templates/
...
if [[ $CRDS_CHANGED -eq 1 ]]; then
    ...
    CRD_CHART_VERSION=$(curl --silent https://raw.githubusercontent.com/giantswarm/rancher-fleet/refs/heads/main/helm/rancher-fleet/charts/rancher-fleet-crds/Chart.yaml | yq .version -r)
fi
