...
|---|---|---|
| `rancher-fleet` | the chart directory `helm/rancher-fleet`, `Chart.yaml`, `values.yaml`, `.abs/main.yaml`, `CHANGELOG.md`, this README | the repository name |
| `rocket` | the `io.giantswarm.application.team` annotation in `Chart.yaml` | the owning team's short name, e.g. `shield` for team-shield |
| `https://github.com/giantswarm/rancher-fleet` | this README | the upstream Helm repository the chart is based on |
...
Giant Swarm offers a rancher-fleet App which can be installed in workload clusters.
Here, we define the rancher-fleet chart with its templates and default configuration.
...
- https://github.com/giantswarm/rancher-fleet
